...
exports.importKey = function importKey(
  ...
  switch (format) {
    ...
    case 'jwk':
      ...
      if ('d' in jwk) {
        const key = Buffer.from(jwk.d, 'base64url').buffer
        ...
        return new CryptoKey(
          ...
          usages,
          new Ed25519PrivateKey(key)
        )
      }

      const key = Buffer.from(jwk.x, 'base64url').buffer
...
exports.exportKey = function exportKey(format, key) {
  ...
  switch (format) {
    ...
    case 'raw': {
      ...
      Buffer.from(result).set(Buffer.from(data._key))
      return result
    }
    case 'jwk': {
